...
<body>
  ...
  <button onclick="updateCompany()">登録</button>
  <script>
  ...
  <script>
    fetch("/api/company/{{$id}}")
      ...
      .then(data => {
        ...
        inputs[1].value = data.companyAddress;
        inputs[2].value = data.phoneNumber;
        inputs[3].value = data.mail;
        textarea.value = data.other;
      });
  </script>
  <script>
    function updateCompany() {
      const data = {
        ...
        companyAddress: inputs[1].value,
        phoneNumber: inputs[2].value,
        mail: inputs[3].value,
        other: textarea.value,
      }
      const param = {
        method: "PUT",
        headers: {
      ...
      fetch("/api/company/{{$id}}", param)
        .then(res => res.json())
        .then(data => {
          window.alert("企業情報が更新されました");
          location.href = "/";
        });
    }
  </script>
